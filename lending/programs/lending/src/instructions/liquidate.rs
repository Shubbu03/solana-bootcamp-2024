...
impl <'info> Liquidate<'info> {
    pub fn liquidate_pool(&mut self, collateral_bank_token_account: u8) -> Result<()>{
        ...
        let health_factor = (total_collateral * collateral_bank.liquadation_threshold)/total_borrowed;

        if health_factor >= 1 {
            return Err(LendingError::NotUndercollateralized.into());
        }

        let liquidation_amount = total_borrowed * collateral_bank.liquadation_close_factor;
        ...
        transfer_checked(cpi_ctx_to_bank, liquidation_amount, self.borrowed_mint.decimals)?;

        // Transfer liquidation value and bonus to liquidator
        let liquidation_bonus = (liquidation_amount * collateral_bank.liquadation_bonus) + liquidation_amount;
        ...
        transfer_checked(cpi_ctx_to_liquidator, liquidation_bonus, self.collateral_mint.decimals)?;
